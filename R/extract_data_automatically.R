...
petfindr_search <- function(token, query = c("animals", "types", 
                                             "organizations")) {
  library(httr)
  library(magrittr)
  library(tidyverse)
  base <- "https://api.petfinder.com/v2/"
  query <- match.arg(query) # this makes the user choose one of the three options
  url <- paste0(base, query)
  search_results <- GET(url = url, 
                        add_headers(Authorization = paste("Bearer", token)))
  
  if(query = "animals") { # IF THE USER SEARCHES FOR ANIMALS
    ...
    return(animal_df)
  } else if(query == "types") { # IF THE USER SEARCHES FOR TYPES OF ANIMALS
    
    type_info <- content(search_results)
    return(type_info)
    
  } else { # IF THE USER SEARCHES FOR ORGANIZATIONS
    
   organization_info <- content(search_results)
