...
Similar to `pf_types` dataset, there is also an example data set of pet breeds available in the package called `pf_breeds` which can be used; however, to have the most recent breeds listed in R, users need to call the function `pf_list_breeds`.
...
Or, a user can conduct time series analysis. Let's create a time series plot to see when the animals are posted on Petfinder.
