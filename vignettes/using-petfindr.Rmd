...
```{r, eval = T, echo = T}
t(LA_puppies %>% head(n = 1))

```

Note the data has been transposed (otherwise the table would be very wide).
...
- specides: Species of the animal
...
The Shiny is a convenient way to explore data through an interactive user interface in the user’s default browser straight from R.\
This section of the vignette will present a couple of examples to show users how to explore Petfinder using R Shiny.\
You also need an authentication to use the Shiny because Shiny extracts information from the API as well.\

The app includes multiple ways to extract and visualize data from Petfinder. All of the output on the Shiny app can be created directly in R as well. \

To run the app, load the `PetFindr` package, and run the `pf_run_Shiny()` function:
...
To ensure that the users register for `key` and `secret`, the main page of the Shiny app 
has a short instruction to let the users know that `key` and `secret` are required to use the Shiny app.\

The shiny app also detects invalid `key` and `secret` and tells the users that the token has to be regenerated every hour. \
